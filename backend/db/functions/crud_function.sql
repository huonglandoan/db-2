USE fooddy;
DELIMITER $$ DROP PROCEDURE IF EXISTS Get_Foods_By_Branch $$ CREATE PROCEDURE Get_Foods_By_Branch(IN p_Branch_ID INT) BEGIN
SELECT sf.*
FROM ServedFood sf
...
BEGIN
    INSERT INTO ServedFood (Food_name, Unit_price, Quantity, Image, Category, Availability_status)
...
DELIMITER $$ DROP PROCEDURE IF EXISTS Update_Food $$ CREATE PROCEDURE Update_Food(
    IN p_Food_ID INT,
    IN p_Name VARCHAR(255),
    IN p_Price DECIMAL(10, 2),
    IN p_Quantity INT,
    IN p_Image VARCHAR(255),
    IN p_Category VARCHAR(50)
) BEGIN -- Cập nhật món ăn
UPDATE ServedFood
SET Food_name = p_Name,
    Unit_price = p_Price,
    Quantity = p_Quantity,
    Image = p_Image,
    Category = p_Category,
